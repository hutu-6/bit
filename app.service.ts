...
export class AppService {
  ...
  getHD() {
    ...
    bip39.mnemonicToSeed(mnemonic).then((seed: any) => {
      ...
      const ecKey = bitcoin.ECKey.fromPrivateKey(Buffer.from(privateKey, 'hex'));
      const privateKeyWIF = ecKey.toWIF();
  ...
  getHDForBip39() {
    ...
    bip39.mnemonicToSeed(mnemonic).then((seed: any) => {
      ...
      //   const privateKey = bitcoin.ECPair.fromPrivateKey(childKey.privateKey);
